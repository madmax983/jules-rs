    ...
    match cli.mode {
        ...
        CommandMode::Multiverse { id_a, id_b } => {
            ...
            loop {
                ...
                activities_a.extend(response.activities);
                match response.next_page_token {
                    Some(token) if !token.is_empty() => page_token_a = Some(token),
                    _ => break,
                }
            }
            activities_a.sort_by(|a, b| {
                a.timestamp
                    .as_deref()
                    .unwrap_or("")
            ...
            loop {
                ...
                activities_b.extend(response.activities);
                match response.next_page_token {
                    Some(token) if !token.is_empty() => page_token_b = Some(token),
                    _ => break,
                }
            }
            activities_b.sort_by(|a, b| {
                a.timestamp
                    .as_deref()
                    .unwrap_or("")
                    .cmp(b.timestamp.as_deref().unwrap_or(""))
