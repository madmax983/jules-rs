...
async fn run_inner(args: Vec<String>) -> Result<Option<String>, DirectorError> {
    ...
    match cli.mode {
        ...
        CommandMode::RiskHeatmap { session_id } => {
            ...
            outputs.extend(session.outputs.clone());
            ...
            let heatmap = jules_rs::ProjectRiskHeatmap::new();
            let workspace_root = env::current_dir()?;
        ...
        CommandMode::Multiverse {
            ...
            let session_a = client.get_session(&session_a_id).await?;
            let mut activities_a = Vec::new();
            let mut page_token = None;
            loop {
                let response = client
            ...
            let session_b = client.get_session(&session_b_id).await?;
            let mut activities_b = Vec::new();
            let mut page_token = None;
            loop {
                let response = client
            ...
            let comparator = jules_rs::SessionComparator::new();
            let report = comparator.compare(&session_a, &activities_a, &session_b, &activities_b);

            let visualizer = jules_rs::MultiverseVisualizer::new();
            let output = visualizer.to_mermaid(
                &report,
                &session_a,
                &activities_a,
                &session_b,
...
fn parse_cli(args: impl IntoIterator<Item = String>) -> Result<Cli, DirectorError> {
    ...
    let mode = match positional[0].as_str() {
        ...
        "scout" => {
            if positional.len() != 1 {
                return Err(DirectorError::Usage(
                    "scout does not accept positional arguments".to_string(),
                ));
            }
            CommandMode::Scout
        }
...
fn parse_multiverse_mode(positional: &[String]) -> Result<CommandMode, DirectorError> {
    ...
    Ok(CommandMode::Multiverse {
        session_a_id: positional[1].clone(),
        session_b_id: positional[2].clone(),
    })
}
