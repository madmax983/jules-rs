...
async fn run_inner(args: Vec<String>) -> Result<Option<String>, DirectorError> {
    ...
    match cli.mode {
        ...
        #[cfg(feature = "estimator")]
        CommandMode::Estimate { session_id } => {
            let api_key = env::var("JULES_API_KEY").map_err(|_| DirectorError::MissingApiKey)?;
            let client = build_client(api_key)?;
            ...
            let estimator = jules_rs::SessionEstimator::new();
...
fn parse_cli(args: impl IntoIterator<Item = String>) -> Result<Cli, DirectorError> {
    ...
    let mode = match positional[0].as_str() {
        ...
        #[cfg(feature = "estimator")]
        "estimate" => parse_estimate_mode(&positional)?,
...
#[cfg(feature = "estimator")]
fn parse_estimate_mode(positional: &[String]) -> Result<CommandMode, DirectorError> {
    if positional.len() != 2 {
        return Err(DirectorError::Usage(
            "estimate requires: estimate <session_id>".to_string(),
        ));
    }
    Ok(CommandMode::Estimate {
        session_id: positional[1].clone(),
    })
}
...
fn usage() -> String {
  ...
  director status [--state <path>] [--json]",
    ...
    #[cfg(feature = "estimator")]
    msg.push_str("\n  director estimate <session_id> [--json]");
