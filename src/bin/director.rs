    ...
    match cli.mode {
        ...
        CommandMode::Multiverse { id_a, id_b } => {
            ...
            loop {
                ...
                let mut page = client.list_activities(&session_a.name, params).await?;
                activities_a.append(&mut page.activities);
            ...
            loop {
                ...
                let mut page = client.list_activities(&session_b.name, params).await?;
                activities_b.append(&mut page.activities);
                match page.next_page_token {
                    Some(token) if !token.is_empty() => page_token = Some(token),
                    _ => break,
                }
            ...
            let report =
                visualizer.to_mermaid(&session_a, &activities_a, &session_b, &activities_b);
            Ok(Some(report))
        }
    }
}
